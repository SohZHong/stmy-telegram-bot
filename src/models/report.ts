...
export async function createReport(
  ...
  const { rows } = await pool.query<Report>(
    `INSERT INTO reports (reporter_telegram_id, reported_telegram_id, reason_id, details)
...
export async function countPendingReportsAgainst(
  ...
  const { rows } = await pool.query<{ count: string }>(
    `SELECT COUNT(*) as count FROM reports
     WHERE reported_telegram_id = $1 AND status = 'pending'`,
    [reportedId],
  );
...
export async function hasRecentReport(
  ...
  const { rows } = await pool.query<{ count: string }>(
    `SELECT COUNT(*) as count FROM reports
     WHERE reporter_telegram_id = $1
       AND reported_telegram_id = $2
...
export async function getMostReportedUsers(
  limit: number,
): Promise<MostReportedUser[]> {
  const { rows } = await pool.query<MostReportedUser>(
    `SELECT reported_telegram_id, COUNT(*)::int as count
     FROM reports
     WHERE status = 'pending'
     GROUP BY reported_telegram_id
     ORDER BY count DESC
     LIMIT $1`,
    [limit],
  );

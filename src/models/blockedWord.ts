...
export async function getBlockedWord(id: number): Promise<BlockedWord | null> {
  const { rows } = await pool.query<BlockedWord>(
...
export async function addBlockedWord(word: string, createdBy: number): Promise<BlockedWord> {
  const { rows } = await pool.query<BlockedWord>(
...
export async function updateBlockedWord(id: number, word: string): Promise<BlockedWord | null> {
  const { rows } = await pool.query<BlockedWord>(
...
export async function deleteBlockedWord(id: number): Promise<boolean> {
  const { rowCount } = await pool.query(`DELETE FROM blocked_words WHERE id = $1`, [id]);
  return (rowCount ?? 0) > 0;
}

export async function countBlockedWords(): Promise<number> {
  const { rows } = await pool.query<{ count: string }>(`SELECT COUNT(*) as count FROM blocked_words`);
